...
namespace Atom.Comn
{
    public partial class fRgstAuto : Form
    {
        ...
        private void RgstCartPay(DataRow row, int rowNo, int rowCnt)
        {
            ...
            try
            {
                ...
                if (html.Contains("과다등기부"))
                {
                    ...
                    //if (drv.FindElement(By.CssSelector("button.btn_bg02_action")).Enabled)
                    if(html.Contains("btn_bg02_action"))
                    {
                        //[다음] 버튼이 있으면 발급 진행
        ...
        private void RgstPrintPdf(string currentWindowHandle)
        {
            ...
            db.Open();
            db.ExeQry($"update db_tank.tx_rgst_auto set rdtm=NOW() where pin='{pin}' and ul=0");     //열람일시 기록
            db.Close();
            ...
            try
            {
                ...
                db.Open();
                db.ExeQry($"update db_tank.tx_rgst_auto set idtm=NOW() where pin='{pin}' and ul=0");     //발급일시 기록
                db.Close();
        ...
        /// 등기 열람(크롬 기반)
        /// </summary>
        private void RgstReading()
        {
            string currentWindowHandle = "", pin = "";
            IWebElement el = null;
            ...
            drv.Navigate().GoToUrl("http://www.iros.go.kr/frontservlet?cmd=RISURetrieveUnissuedListC&unvRfrYn=Y");
            Thread.Sleep(5000);
            ...
            while (true)
            {
                ...
                try
                {
                    if (drv.PageSource.Contains("열람/발급가능한 부동산이 존재하지 않습니다")) break;
                    IReadOnlyCollection<IWebElement> ecTr = drv.FindElements(By.XPath(@"//*[@id='Lcontent']/form[1]/div[5]/table/tbody/tr"));
                    if (ecTr == null) break;
                    if (ecTr.Count() < 2) break;

                    el = ecTr.ElementAt(1);
                    pin = Regex.Replace(el.FindElements(By.TagName("td"))[4].Text, @"[^\d]", string.Empty);

                    db.Open();
                    db.ExeQry($"update db_tank.tx_rgst_auto set rdtm=NOW() where pin='{pin}' and ul=0");     //열람일시 기록
                    db.Close();

                    ExportPdf(1, el, pin, currentWindowHandle);
                }
                catch (Exception ex)
                {
                    if (ex.Message.Contains("삭제된 개체"))
        ...
        /// </summary>
        /// <param name="dt"></param>
        private void IssueFailRetry()
        {
            string currentWindowHandle = "", pin = "", sql;
            decimal rgCnt, pgCnt;
            IWebElement el = null;
            string today = DateTime.Now.ToShortDateString();
            ...
            if (drv == null) return;
            if (drv.PageSource.Contains("재열람 가능한 부동산이 없습니다")) return;
            IWebElement elRgCnt = drv.FindElement(By.CssSelector("pg_sum"));
            if (elRgCnt.Displayed)
            {
                Match match = Regex.Match(elRgCnt.Text, @"총[ ]*(\d+)[ ]*건");
                if (!match.Success) return;
                rgCnt = Convert.ToInt16(match.Groups[1].Value);
                if (rgCnt == 0) return;
                pgCnt = Math.Ceiling(rgCnt / (decimal)100);
            }
            else
            {
                return;
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                for (int i = 1; i <= rgCnt; i++)
                {
                    ...
                    foreach (IWebElement elTr in ecTr)
                    {
                        pin = Regex.Replace(elTr.FindElements(By.TagName("td"))[4].Text, @"[^\d]", string.Empty);
                        if (pin == row["pin"].ToString())
                        {
                            el = elTr;
                            /*
                            ClickElementSafe(drv, elTr.FindElement(By.Name("chkSummary")), 30);
                            Thread.Sleep(1500);
                            ClickElementSafe(drv, elTr.FindElement(By.Name("inpView")), 30);
                            Thread.Sleep(1500);
                            bool srchRslt = imgSrchClick(@"C:\Atom\btnXCtrl.bmp");
                            if (!srchRslt)
                            {
                                Thread.Sleep(1500);
                                srchRslt = imgSrchClick(@"C:\Atom\btnXCtrl.bmp");
                                if (!srchRslt)
                                {
                                    if (at.WinExists("재열람 목록") == 1)
                                    {
                                        at.MouseClick("LEFT", (Convert.ToInt16((at.WinGetPosWidth("재열람 목록") * 0.5)) + 50), 200);
                                    }
                                }                                
                            }
                            */
                ...
                if (el == null || pin == string.Empty) continue;

                EXIT:
                    ExportPdf(2, el, pin, currentWindowHandle);
            }
            drv.Close();
            drv.Quit();
        }
        ...
        private void ExportPdf(int exportCase, IWebElement el, string pin, string currentWindowHandle)
        {
            int rowCnt = 0, pdfPrcWait = 0;
            long timeStamp = 0;
            string idx, locFile, winTitle;
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (el.FindElement(By.Name("chkSummary")).Enabled)
                    {
                        ClickElementSafe(drv, el.FindElement(By.Name("chkSummary")), 30);
                        Thread.Sleep(1500);
                    }
                    ClickElementSafe(drv, el.FindElement(By.CssSelector("td:nth-child(11) > button")), 30);
                    ...
                    if (!srchRslt)
                    {
                        Thread.Sleep(1500);
                        srchRslt = imgSrchClick(@"C:\Atom\btnXCtrl.bmp");
                        winTitle = (exportCase == 1) ? "미열람/미발급 보기" : "재열람 목록";
                        if (!srchRslt)
                        {
                            if (at.WinExists(winTitle) == 1)
                            {
                                at.MouseClick("LEFT", (Convert.ToInt16((at.WinGetPosWidth(winTitle) * 0.5)) + 50), 200);
                            }
                        }
                    }

                    if (drv.SwitchTo().Alert().Text.Contains("신청사건"))
                    {
                        drv.SwitchTo().Alert().Accept();
                        db.Open();
                        MySqlDataReader dr = db.ExeRdr($"select idx from db_tank.tx_rgst_auto where wdt > date_sub(curdate(),INTERVAL 10 day) and pin='{pin}' order by idx desc limit 1");
                ...
                if (at.WinExists("테스트열람") == 1)
                {
                    ...
                    if (drv.WindowHandles.Count > 0)
                    {
                        ...
                        {
                            drv.SwitchTo().Window(winNm);
                            if (drv.Title.Contains("테스트열람"))
                            {
                                Thread.Sleep(1000);
                                //this.js.ExecuteScript("javascript:f_goTestView(); return false;", Array.Empty<object>());
                                drv.FindElement(By.XPath(@"//*[@id='content1']/div[2]/div/div/a/strong")).Click();
                                Thread.Sleep(3000);
            ...
            catch (Exception ex)
            {
                ...
                if (exportCase == 2) IssueFailRetry();
                else RgstIssue();
            }
        }
        ...
        {
            string sql, cDtm, tid = "", lsType = "", rgstYN = "", rgState = "";
            int dvsn = 0;
            if (idx == "")
            {
            ...
            if (errCd == 20 && idx != "")
            {
                sql = $"select * from db_tank.tx_rgst_auto where idx='{idx}' limit 1";
                MySqlDataReader dr = db.ExeRdr(sql);
                dr.Read();
                if (dr.HasRows)
                {
                    tid = dr["tid"].ToString();
                    dvsn = Convert.ToInt16(dr["dvsn"]);
                    lsType = dr["ls_type"].ToString();
                }
                dr.Close();
                                
                if (dvsn == 11 || dvsn == 12 || dvsn == 14) //경매-일반, 선행, 예정
                {
                    sql = $"select rgst_yn from ta_list where tid='{tid}' limit 1";
                    dr = db.ExeRdr(sql);
                    dr.Read();
                    if (dr.HasRows)
                    {
                        rgstYN = dr["rgst_yn"].ToString();
                    }
                    dr.Close();

                    if (rgstYN == "0") rgState = (lsType.Contains("토지")) ? "11" : "10";         //토지 : 건물
                    else if (rgstYN == "11") rgState = (lsType.Contains("토지")) ? "" : "12";     //변동없음 : 토+건
                    else if (rgstYN == "10") rgState = (lsType.Contains("토지")) ? "12" : "";     //토+건 : 변동없음

                    if (rgState != "")
                    {
                        sql = $"update ta_list set rgst_yn='{rgState}' where tid='{tid}'";
                        db.ExeQry(sql);
                    }
                }
                else if (dvsn >= 20)    //공매
                {
                    sql = $"select rgst_yn from tb_list where cltr_no='{tid}' limit 1";
                    dr = db.ExeRdr(sql);
                    dr.Read();
                    if (dr.HasRows)
                    {
                    ...
                    if (rgState != "")
                    {
                        sql = $"update tb_list set rgst_yn='{rgState}' where cltr_no='{tid}'";
                        db.ExeQry(sql);
                    }
                }
            }

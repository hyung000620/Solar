...
namespace Solar.CA
{
    public partial class wfNtManual : Form
    {
        ...
        private void Prc_Lst(string crt, string spt, string dpt, string bidDt, string jiwonNm, DataTable dtSa)
        {
            ...
            int rowIdx = 0, lotCnt = 0, creditorCnt = 0, debtorCnt = 0, ownerCnt = 0;
            bool eqFlag = false;
            ...
            foreach (DataRow row in dtSa.Rows)
            {
                ...
                db.Open();
                sql = "select tid from ta_list where spt=" + spt + " and sn1=" + sn1 + " and sn2=" + sn2 + " and sta2=1010 limit 1";
        ...
        private void Prc_Dtl()
        {
            ...
            string bidCnt, bidDt, bidTm1, bidTm2, bidTm3, minbAmt1, minbAmt2;
            ...
            sql = "select tid, crt, spt, sn1, sn2, pn, bid_dt, minb_amt, frml_type from ta_list where 2nd_dt='" + srch2ndDt + "' and spt='" + srchSpt + "' and dpt='" + srchDpt + "' and auto_prc=1 order by tid";
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                foreach (HtmlNode tr in ncTr)
                {
                    ...
                    if (Regex.IsMatch(ncTd[0].InnerText, @"목록\d+\s+소재지"))
                    {
                ...
                if (listCat.Count > 0)
                {
                    ...
                    if (matchCar.Success) PrcDtlSub_Car(tid, doc, matchCar.Value, tblApsl);
                    //else if (matchHeavyEquip.Success) PrcDtlSub_HeavyEquip(tid, doc, matchHeavyEquip.Value);
                    else if (matchHeavyEquip.Success) PrcDtlSub_Car(tid, doc, matchHeavyEquip.Value, tblApsl);
                    else if (matchShip.Success) PrcDtlSub_Ship(tid, doc);
                    else if (matchAir.Success) PrcDtlSub_Air(tid, doc);
                    else if (matchBike.Success) PrcDtlSub_Bike(tid, doc);
                    else if (matchFish.Success) PrcDtlSub_Fish(tid, doc, tblApsl, pdNote);
                    else if (matchMine.Success) PrcDtlSub_Mine(tid, doc, tblApsl);
                    else
                    {
                        landBldgFlag = true;
                        PrcDtlSub_LandBldg(tid, doc, tblApsl);
                    }
                }
                else
                {
                ...
                if (cat3 != string.Empty)
                {
                    ...
                    if (lsNode != null)
                    {
                        dpslDvsn = Dpsl_DvsnCd(pdNote, lsNode);
                    }
                ...
                //건물현황 현황(이용상태) 업데이트 - ? 제시외 건물 이용상태와 충돌남 확인 필요
                //useSta = Regex.Replace(useSta, @"\s+", " ");
                //sql = "update ta_bldg set state='" + useSta + "' where tid=" + tid;
                //db.ExeQry(sql);
            ...
            foreach (DataRow row in dtList.Rows)
            {
                ...
                if (ncTbl != null)
                {
                    foreach (HtmlNode ndTbl in ncTbl)
                    {
                        ...
                        //if (dtLs.Select("no='" + lsNo + "' and dvsn <> '건물' and dvsn <> '집합건물'").Count() == 0) continue;
                        if (dtLs.Select("no='" + lsNo + "'").Count() == 0) continue;
                ...
                if (alEtc.Count > 0)
                {
                    ...
                    sql = "update ta_dtl set leas_note='" + etc + "' where tid='" + tid + "'";
                    db.ExeQry(sql);
                }
                ...
                foreach (DataRow data in dtT.Rows)
                {
                    ...
                    //sp.Add(new MySqlParameter("@deposit", data["deposit"]));
                    //sp.Add(new MySqlParameter("@m_money", data["fee"]));
        ...
        private void Prc_RgstAnaly()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                try
                {
                    ...
                    RgstAnaly rgstAnaly = new RgstAnaly();
        ...
        {
            int i = 0, curCnt = 0, totCnt = 0;
            string sql, tbl, cvp, url, jiwonNm, spt, dpt, sn, sn1, sn2, bidDt, saNo, pn, html, html0, alt, dir, ctgr, year, fileNm, locFile, rmtFile, thumb, locThumbFile, rmtThumbFile, seq;
            ...
            sql = "select tid,crt,spt,dpt,sn1,sn2,pn,bid_dt from ta_list where 2nd_dt='" + srch2ndDt + "' and spt='" + srchSpt + "' and dpt='" + srchDpt + "' and auto_prc in (2,3) group by crt,spt,sn1,sn2 order by spt,dpt,sn1,sn2";
            DataTable dt = db.ExeDt(sql);
            totCnt = dt.Rows.Count;
            if (totCnt == 0)
            {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                sql = "select tid from ta_list where spt=" + spt + " and sn1=" + sn1 + " and sn2=" + sn2 + " and 2nd_dt < '" + srch2ndDt + "'";
                db.Open();
        ...
        private void PrcDtlSub_LandBldg(string tid, HAPDoc doc, HtmlNode tblApsl)
        {
            ...
            foreach (HtmlNode tr in ncTr)
            {
                ...
                {
                    PrcDtlSub_Car(tid, doc, lsType, tblApsl);
                    break;
                }
        ...
        private void PrcDtlSub_Car(string tid, HAPDoc doc, string cat, HtmlNode tblApsl)
        {

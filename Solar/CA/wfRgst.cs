...
namespace Solar.CA
{
    public partial class wfRgst : Form
    {
        ...
        private void init()
        {
            ...
            dtA = new DataTable();  //분석-전
            dtB = new DataTable();  //분석-후
        ...
        private void dgF_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                row.Cells["dgF_Note"].Value = ex.Message;
            }
            finally
            {
        ...
        {
            if (MessageBox.Show("모든 파일에 대하여 일괄처리를 하시겠습니까?", "등기추출-일괄처리", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Cancel)
            {
                return;
            }
            dgA.DataSource = null;
            dgB.DataSource = null;
            flagBatchProc = true;
            btnBatProc.Enabled = false;
            btnBatProc.BackColor = Color.LightPink;
            ...
            btnBatProc.Enabled = true;
            btnBatProc.BackColor = Color.LightGreen;
            flagBatchProc = false;

            MessageBox.Show("일괄처리가 완료 되었습니다.");
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (dgF.CurrentRow == null || dgF.SelectedRows.Count == 0)
            {
                MessageBox.Show("물건을 선택 해 주세요");
                return;
            }

            rowIdx = dgF.CurrentRow.Index;

            ProcStep4(rowIdx);
        }

        private void ProcStep1(int rowIdx)
        {
            string pdfFile, htmlFile, html, rgstIdNo, rgstDvsn;
            ...
            //listStrip.Add(@"^<div.*ft[13457890]+"">(\[(집합건물|토지|건물)\]|고유번호|관할등기소|[\s\-]+이\s+하\s+여\s+백|[\s\*]+(본 등기사항증명서는|증명서는|실선으로|기록사항 없는)|[<b>]*(출력|열람)일시|[<b>]*\[\s(참 고 사 항|주 의 사 항)\s\]|<b>[본\s]*주요 등기사항|\(\s+(소유권에 관한|소유권 이외의)|가[\.\s]+등기기록에서).*/div>");
            ...
            pdfFile = dgF["dgF_Nm", rowIdx].Value.ToString();
            ...
            //Bullzip Pdf
            //rgstDvsn = Regex.Match(html, @"<br>- (토지|건물|집합건물) -</span></nobr>", rxOptM).Groups[1].Value;
            rgstIdNo = Regex.Match(html, @"고유번호 (\d{4}-\d{4}-\d{6})", rxOptM).Groups[1].Value;
            if (rgstDvsn == string.Empty)
            {
                throw new Exception("등기 구분 불가");
            }
            dgF["dgF_Dvsn", rowIdx].Value = rgstDvsn;   //등기 구분
            dgF["dgF_IdNo", rowIdx].Value = rgstIdNo;   //등기 고유번호
        ...
        private void ProcStep2(string sect, int bgnIdx, int endIdx, MatchCollection mc)
        {
            ...
            //Bullzip Pdf
            //int rankEnd = 71, prpsEnd = 172, rcDtEnd = 252, resnEnd = 333;  //순위번호, 등기목적, 접수, 등기원인, 권리자 및 기타사항(마지막 칼럼)
            //int prsnEnd = 106, rgNoEnd = 187, shrEnd = 252, adrsEnd = 520;  //등기명의인, (주민)등록번호, 최종지분, 주소, 순위번호(마지막 칼럼)
        ...
        private void ProcStep3()
        {
            ...
            int rankLen = 0, shrCnt = 0, grpCnt = 0, n = 0;
            bool chkPrsn = false;
            ...
            if (!flagBatchProc)
            {
                ...
                {
                    if (row.Cells["dgB_del"].Value.ToString() == "1") row.DefaultCellStyle.ForeColor = Color.LightGray;
                    if (row.Cells["dgB_ekey"].Value.ToString() == "1") row.DefaultCellStyle.BackColor = Color.LightGreen;
                }
            }
        }
        ...
        /// DB 처리
        /// </summary>
        private void ProcStep4(int rowIdx)
        {
            string sql, cvp, tid, dvsn, pin, dvsnCd, sect, rank, rank_s, filter = "0";
            string saNo = "", billAmt = "", bAmt = "", aply = "", take = "", note = "";
            bool eKey = false;
            List<MySqlParameter> sp = new List<MySqlParameter>();
            Match match;

            tid = dgF["dgF_Tid", rowIdx].Value.ToString();
            dvsn = dgF["dgF_Dvsn", rowIdx].Value.ToString();
            saNo = dgF["dgF_SaNo", rowIdx].Value.ToString();
            pin = dgF["dgF_IdNo", rowIdx].Value.ToString();

            if (dvsn == "토지") dvsnCd = "1";
            else if (dvsn == "건물") dvsnCd = "2";
            else if (dvsn == "집합건물") dvsnCd = "3";
            else return;

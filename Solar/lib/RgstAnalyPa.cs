...
namespace Solar
{
    ...
    public class RgstAnalyPa
    {
        ...
        public string Proc(string pdfFile, bool dbPrc = false)
        {
            string sn1, sn2, sql;

            this.dbPrc = dbPrc;

            this.cltrNo = string.Empty;
            ...
            //Regex rx = new Regex(@"[IJ]*(\d+)(_\d)*(\-\d)*\.pdf", rxOptM);
            Regex rx = new Regex(@"^[IJ]*(\d{5,7})(_\d)*(\-\d)*\.pdf", rxOptM);  //2021-10-27 공매등기 자체 발급(이용기관만)
            Regex rx3 = new Regex(@"(\d{14})\.pdf", rxOptM);                    //2022-04-14 모든 등기 자체 발급
            try
            {
                db.Open();
                Match match = rx.Match(pdfFile);
                Match match3 = rx3.Match(pdfFile);
                if (match.Success)
                {   
                    sql = "select cltr_no from tb_list where cltr_no='" + match.Groups[1].Value + "' limit 1";
                    MySqlDataReader dr = db.ExeRdr(sql);
                    dr.Read();
                    if (dr.HasRows) this.cltrNo = dr["cltr_no"].ToString();
                    else this.cltrNo = string.Empty;
                    dr.Close();
                }
                else if (match3.Success)
                {
                    //sql = $"select * from db_tank.tx_rgst_auto R, db_main.tb_list L where R.tid=L.cltr_no and wdt=curdate() and pin='{match3.Groups[1].Value}' and (R.dvsn between 20 and 23) and ul=0";
                    sql = $"select * from db_tank.tx_rgst_auto R, db_main.tb_list L where R.tid=L.cltr_no and wdt > date_sub(curdate(),INTERVAL 10 day) and pin='{match3.Groups[1].Value}' and (R.dvsn between 20 and 23) and ul=0";
                    MySqlDataReader dr = db.ExeRdr(sql);
                    dr.Read();
                    if (dr.HasRows)
                    {
                        this.cltrNo = dr["tid"].ToString();
                    }
                    else
                    {
                        this.cltrNo = string.Empty;
                ...
                if (cltrNo == string.Empty) throw new Exception("해당 cltrNo 없음");
        ...
        {
            string htmlFile, html, rgstIdNo, rgstDvsn;
        ...
        {
            string sql, cvp, dvsnCd, sect, rank, rank_s, filter = "0";
            ...
            if (dvsnCd == "1")
            {
                sql = "delete from tb_regist where cltr_no=" + cltrNo + " and rg_dvsn=1";
            }
            else
            {
                sql = "delete from tb_regist where cltr_no=" + cltrNo + " and rg_dvsn IN (2,3)";
            }
            db.ExeQry(sql);
            db.Close();
            ...
            */
            ...
            foreach (DataRow row in dtB.Rows)
            {
                ...
                sql = "insert into tb_regist set cltr_no=@cltr_no, rg_dvsn=@rg_dvsn, sect=@sect, rank=@rank, rank_s=@rank_s, rg_cd=@rg_cd, rg_nm=@rg_nm, rc_dt=@rc_dt, rc_no=@rc_no, b_amt=@b_amt, c_amt=@c_amt, prsn=@prsn, rg_no=@rg_no, mv_dt=@mv_dt, fx_dt=@fx_dt, bgn_dt=@bgn_dt, end_dt=@end_dt, ";
            ...
            sp.Add(new MySqlParameter("@tid", cltrNo));
            ...
            */
            ...
            sql = "update tb_dtl set " + cvp + " where cltr_no=" + cltrNo;
            db.ExeQry(sql);
            db.Close();
